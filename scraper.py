...
from researcher.api.prompts import *
...
class Scraper:
    ...
    """
    def __init__(self, user_agent, cfg):
        """
        ...
        self.retriever = self.get_retriever(self.cfg.retriever) 
    ...
    def scrape_urls(self, urls):
        ...
        """
        content = []
        user_agent = self.cfg.user_agent if self.cfg else "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36 Edg/119.0.0.0"
        try:
            content = self.run(urls)
    ...
    def get_retriever(self, retriever):
        ...
        match retriever:
            case "tavily":
                from researcher.retrievers import TavilySearch
                retriever = TavilySearch
            case "tavily_news":
                from researcher.retrievers import TavilyNews
                retriever = TavilyNews
            case "google":
                from researcher.retrievers import GoogleSearch
                retriever = GoogleSearch
            case "searx":
                from researcher.retrievers import SearxSearch
            ...
            case "BingSearch":
                from researcher.retrievers import BingSearch
